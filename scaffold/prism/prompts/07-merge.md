...
3. Конфликты слоёв: по owner (напр. 03 vs 04) побеждает **04**; прочие пересечения (напр. `02` do-not-touch против правки по 03/04) → задача с пометкой «conflict» + зависимостью, решение в «Conflict resolutions» и (при необходимости) у владельца слоя; агент не решает в одиночку.
